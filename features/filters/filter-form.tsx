...
import { Autocomplete, Box, Checkbox, Chip, FormControl, FormControlLabel, FormGroup, FormLabel, InputAdornment, MenuItem, Select, Stack, Switch, TextField, Typography } from "@mui/material";
...
type DictState = { items: DictionaryItem[]; loading: boolean };

const EMPTY_DICT: DictState = { items: [], loading: false };

function useDictionary(path: string, deps: string[] = []) {
  const [state, setState] = useState<DictState>(EMPTY_DICT);
  ...
  const load = useCallback(async (q?: string) => {
    ...
    try {
      ...
      const data = await apiRequest<{ items: DictionaryItem[] }>(`${path}${qs ? `?${qs}` : ""}`);
...
function useDictionaryItems(code: string) {
  const [state, setState] = useState<DictState>(EMPTY_DICT);
  const loaded = useRef(false);
  const load = useCallback(async (q?: string) => {
    setState((s) => ({ ...s, loading: true }));
    try {
      const params = new URLSearchParams();
      params.set("dictionary_code", code);
      if (q) params.set("q", q);
      params.set("limit", "50");
      const data = await apiRequest<{ items: DictionaryItem[] }>(`/api/dictionaries/dictionary-items?${params.toString()}`);
      setState({ items: data?.items ?? [], loading: false });
    } catch {
      setState((s) => ({ ...s, loading: false }));
    }
