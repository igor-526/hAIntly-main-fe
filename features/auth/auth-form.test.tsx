...
vi.mock("./auth-provider",()=>({useAuth:()=>({check})}));
vi.mock("./service",()=>({authService:{register:vi.fn(),login:vi.fn()}}));
async function registration(userReturn?:ReturnType<typeof authService.register>){
 if(userReturn)vi.mocked(authService.register).mockReturnValue(userReturn);else vi.mocked(authService.register).mockResolvedValue(undefined);const user=userEvent.setup();render(<AuthForm/>);await user.click(screen.getByRole("button",{name:"Регистрация"}));await user.type(screen.getByLabelText("Email"),"a@b.ru");await user.type(screen.getByLabelText("Пароль"),"password1");return user;
}
describe("AuthForm interactions",()=>{
 ...
 it("блокирует submit в pending",async()=>{let resolve!:()=>void;const user=await registration(new Promise<void>(r=>{resolve=r}));await user.click(screen.getByRole("button",{name:"Зарегистрироваться"}));expect(screen.getByRole("button",{name:"Подождите…"})).toBeDisabled();expect(authService.register).toHaveBeenCalledOnce();resolve();await waitFor(()=>expect(screen.getByRole("status")).toBeInTheDocument())});
});
