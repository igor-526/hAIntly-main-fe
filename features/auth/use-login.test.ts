...
describe("useLogin",()=>{
 ...
 it("регистрирует без auto-login и переносит email",async()=>{vi.mocked(authService.register).mockResolvedValue(undefined);const {result}=renderHook(()=>useLogin());act(()=>{result.current.switchMode("register");result.current.setRegister({email:"a@b.ru",password:"password1"})});await act(async()=>result.current.submit());expect(authService.register).toHaveBeenCalledWith({email:"a@b.ru",password:"password1"});expect(authService.login).not.toHaveBeenCalled();expect(result.current.mode).toBe("login");expect(result.current.login.email).toBe("a@b.ru");expect(result.current.success).toContain("Пользователь создан")});
 it("после login проверяет session и навигирует",async()=>{vi.mocked(authService.login).mockResolvedValue(undefined);check.mockResolvedValue(true);const {result}=renderHook(()=>useLogin());act(()=>result.current.setLogin({email:"tester@example.test",password:"password1"}));await act(async()=>result.current.submit());expect(check).toHaveBeenCalledOnce();expect(replace).toHaveBeenCalledWith("/")});
