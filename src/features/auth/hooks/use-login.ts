...
export function useLogin() {
  ...
  const submit = async () => {
    if (pending) return;
    const nextErrors = mode === "login" ? validateLogin(login) : validateRegister(register);
    ...
    try {
      if (mode === "register") {
        ...
        setRegister((v) => ({ ...v, password: "" }));
        setMode("login");
        setSuccess("Пользователь создан. Теперь войдите.");
      } else {
        await authService.login(login);
        if (await auth.check()) router.replace("/");
        else setSubmitError("Не удалось подтвердить сессию.");
      }
    } catch (error) {
      setSubmitError(error instanceof Error ? error.message : "Запрос не выполнен");
    } finally {
      setPending(false);
    }
  };
