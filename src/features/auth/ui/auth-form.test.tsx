...
async function registration(userReturn?: ReturnType<typeof authService.register>) {
  if (userReturn) vi.mocked(authService.register).mockReturnValue(userReturn);
  else vi.mocked(authService.register).mockResolvedValue(void 0);
...
describe("AuthForm interactions", () => {
  ...
  it("переключает реальные режимы", async () => {
    const user = userEvent.setup();
    render(<AuthForm />);
    await user.click(screen.getByRole("button", { name: "Регистрация" }));
  ...
  it("показывает validation без запроса", async () => {
    const user = userEvent.setup();
    render(<AuthForm />);
    await user.click(screen.getByRole("button", { name: "Войти" }));
    expect(await screen.findAllByText(/Введите/)).toHaveLength(2);
    expect(authService.login).not.toHaveBeenCalled();
  });
  it("показывает email и пароль в обоих режимах", async () => {
    const user = userEvent.setup();
    render(<AuthForm />);
  ...
  it("показывает success регистрации, переносит email и не делает login", async () => {
    const user = await registration();
    await user.click(screen.getByRole("button", { name: "Зарегистрироваться" }));
    expect(await screen.findByRole("status")).toHaveTextContent("Пользователь создан");
    expect(screen.getByLabelText("Email")).toHaveValue("a@b.ru");
    expect(authService.login).not.toHaveBeenCalled();
  });
  it("показывает backend error", async () => {
    const user = await registration();
    vi.mocked(authService.register).mockRejectedValueOnce(new Error("Email занят"));
    await user.click(screen.getByRole("button", { name: "Зарегистрироваться" }));
    expect(await screen.findByRole("alert")).toHaveTextContent("Email занят");
  });
  it("блокирует submit в pending", async () => {
