...
describe("auth service", () => {
  ...
  it("точно сериализует email/password для регистрации и входа", async () => {
    ...
    expect(apiRequest).toHaveBeenNthCalledWith(1, "/api/auth/register", {
      method: "POST",
      body: JSON.stringify({ email: "a@b.ru", password: "password1" }),
    });
    await authService.login({ email: "a@b.ru", password: "password1" });
    expect(apiRequest).toHaveBeenNthCalledWith(2, "/api/auth/token", {
      method: "POST",
      body: JSON.stringify({ email: "a@b.ru", password: "password1" }),
    });
  ...
  it("сохраняет orchestration refresh и verify", async () => {
    await authService.refresh();
    await authService.verify();
    expect(vi.mocked(apiRequest).mock.calls.map((x) => x[0])).toEqual(["/api/auth/refresh", "/api/auth/verify"]);
  });
  it("отправляет logout через существующий main-be endpoint", async () => {
