  ...
  const loadPresets = useCallback(async (q?: string) => {
    ...
    try {
      ...
      setState((s) => ({
        ...s,
        loading: false,
        error: error instanceof Error ? error.message : "Ошибка загрузки пресетов",
      }));
    }
  }, []);
  ...
  const selectPreset = useCallback(async (id: string | null) => {
    ...
    try {
      ...
      setState((s) => ({
        ...s,
        loading: false,
        error: error instanceof Error ? error.message : "Ошибка загрузки пресета",
      }));
    }
  }, []);
      ...
      try {
        ...
        if (state.selectedPresetId) {
          ...
          setState((s) => ({
            ...s,
            saving: false,
            toast: `Пресет «${formName}» успешно сохранён`,
          }));
        } else {
        ...
        setState((s) => ({
          ...s,
          saving: false,
          error: error instanceof Error ? error.message : "Ошибка сохранения пресета",
        }));
        return false;
      }
    },
    ...
    async (id: string, name: string) => {
      ...
      try {
        ...
        setState((s) => ({
          ...s,
          saving: false,
          toast: `Пресет переименован в «${name}»`,
        }));
        await loadPresets();
        return true;
      } catch (error) {
        setState((s) => ({
          ...s,
          saving: false,
          error: error instanceof Error ? error.message : "Ошибка переименования пресета",
        }));
        return false;
      }
    },
    ...
    async (id: string) => {
      ...
      try {
        ...
        setState((s) => ({
          ...s,
          saving: false,
          error: error instanceof Error ? error.message : "Ошибка удаления пресета",
        }));
        return false;
      }
    },
