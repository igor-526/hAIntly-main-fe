  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        refreshPromise ??= authSessionApi.refresh().finally(() => {
          refreshPromise = null;
        });
        try {
          ...
          if (refreshError instanceof ApiError && refreshError.status === 401) {
            setState({ status: "anonymous", logoutPending: false });
            return false;
          }
          throw refreshError;
        }
      ...
      if (user) {
        setState({ status: "authenticated", user, logoutPending: false });
        return true;
      }
      setState({ status: "anonymous", logoutPending: false });
      return false;
  ...
  const logout = useCallback(async () => {
    ...
    try {
      ...
      setState((value) => ({
        ...value,
        logoutPending: false,
        logoutError: error instanceof Error ? error.message : "Не удалось выйти",
      }));
      return false;
    } finally {
...
export function useAuth() {
  ...
  if (!value) throw new Error("useAuth требует AuthProvider");
  return value;
}
